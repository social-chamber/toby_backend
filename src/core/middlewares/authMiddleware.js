...
const adminMiddleware = (req, res, next) => {
  ...
  if (role !== "ADMIN") {
    return generateResponse(res, 403, false, 'Admin access only', null);
  }
...
const lenderMiddleware = (req, res, next) => {
  ...
  if (role !== "LENDER") {
    return generateResponse(res, 403, false, 'Lender access only', null);
  }
...
const adminLenderMiddleware = (req, res, next) => {
  ...
  if (role !== RoleType.ADMIN && role !== RoleType.LENDER) {
    return generateResponse(res, 403, false, 'Admin or Lender access only', null);
  }
